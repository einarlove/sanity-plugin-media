...
function AutoTagInputWrapper(props) {
  ...
  return !isAssetField || !mediaTags || mediaTags.length === 0 ? renderDefault(props) : /* @__PURE__ */ jsxRuntime.jsx(AutoTagAssetInput, { ...props, mediaTags });
}
function AutoTagAssetInput(props) {
  const { renderDefault, value, mediaTags } = props, client = sanity.useClient({ apiVersion: "2022-10-01" }), { createTagsOnUpload } = useToolOptions(), prevAssetRef = react.useRef(void 0), isInitialMount = react.useRef(!0), currentAssetRef = value?.asset?._ref;
...
const UPLOADS_ACTIONS = {
  ...
  autoTagRequest: toolkit.createAction(
  ...
  autoTagComplete: toolkit.createAction(
  ...
  autoTagError: toolkit.createAction(
    "uploads/autoTagError",
    function({ assetId, error }) {
      return {
...
const createThrottler = (concurrency = DEFAULT_CONCURRENCY) => {
  ...
  operators$1.mergeMap((action) => {
    const { asset, createTagsOnUpload, mediaTags } = action.payload, actions = [
      uploadsActions.checkRequest({
        assets: [asset]
      })
    ];
    return mediaTags && mediaTags.length > 0 && actions.push(
      UPLOADS_ACTIONS.autoTagRequest({
        assetId: asset._id,
        createTagsOnUpload: createTagsOnUpload ?? !0,
        mediaTags
      })
    ), rxjs.of(...actions);
  })
...
), uploadsAutoTagEpic = (action$, _state$, { client }) => action$.pipe(
  operators$1.filter(UPLOADS_ACTIONS.autoTagRequest.match),
  operators$1.mergeMap((action) => {
    const { assetId, createTagsOnUpload, mediaTags } = action.payload, resolveTag$ = (tagName) => rxjs.from(
      client.fetch(
        groq__default.default`*[_type == "${TAG_DOCUMENT_NAME}" && name.current == $tagName][0]`,
        { tagName }
      )
    ).pipe(
      operators$1.mergeMap((existingTag) => existingTag ? rxjs.of(existingTag) : createTagsOnUpload ? rxjs.from(
        client.create({
          _type: TAG_DOCUMENT_NAME,
          name: {
            _type: "slug",
    ...
    return rxjs.forkJoin(mediaTags.map(resolveTag$)).pipe(
      operators$1.mergeMap((tags) => {
        const validTags = tags.filter((tag) => tag !== null);
        if (validTags.length === 0)
          return rxjs.of(UPLOADS_ACTIONS.autoTagComplete({ assetId }));
        const tagReferences = validTags.map((tag) => ({
          _key: nanoid.nanoid(),
          _ref: tag._id,
          _type: "reference",
        ...
        return rxjs.from(
          client.patch(assetId).setIfMissing({ opt: {} }).setIfMissing({ "opt.media": {} }).setIfMissing({ "opt.media.tags": [] }).append("opt.media.tags", tagReferences).commit()
        ).pipe(
          operators$1.mergeMap(() => rxjs.of(UPLOADS_ACTIONS.autoTagComplete({ assetId }))),
          operators$1.catchError(
            (error) => rxjs.of(
...
const UploadDropzone = (props) => {
  const { children } = props, {
    dropzone: { maxSize },
    directUploads
  } = useToolOptions(), { onSelect, schemaType } = useAssetSourceActions(), mediaTags = schemaType?.options?.mediaTags, { createTagsOnUpload } = useToolOptions(), dispatch = reactRedux.useDispatch(), assetTypes = useTypedSelector((state) => state.assets.assetTypes), isImageAssetType = assetTypes.length === 1 && assetTypes[0] === "image", handleDrop = async (acceptedFiles) => {
