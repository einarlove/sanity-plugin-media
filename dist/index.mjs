...
import { createContext, useMemo, useContext, useRef, useEffect, useCallback, useState, forwardRef, memo, Component } from "react";
...
const createThrottler = (concurrency = DEFAULT_CONCURRENCY) => {
  ...
  mergeMap((action) => {
    const { assetId, createTagsOnUpload, mediaTags } = action.payload, resolveTag$ = (tagName) => from(
      client.fetch(
        groq`*[_type == "${TAG_DOCUMENT_NAME}" && name.current == $tagName][0]`,
        { tagName }
      )
    ).pipe(
      mergeMap((existingTag) => existingTag ? of(existingTag) : createTagsOnUpload ? from(
        client.create({
          _type: TAG_DOCUMENT_NAME,
          name: {
            _type: "slug",
...
const UploadDropzone = (props) => {
  const { children } = props, {
    dropzone: { maxSize },
    directUploads
  } = useToolOptions(), { onSelect, schemaType } = useAssetSourceActions(), mediaTags = schemaType?.options?.mediaTags, { createTagsOnUpload } = useToolOptions(), dispatch = useDispatch(), assetTypes = useTypedSelector((state) => state.assets.assetTypes), isImageAssetType = assetTypes.length === 1 && assetTypes[0] === "image", handleDrop = async (acceptedFiles) => {
